...
export const parseMultipartFormData = (
  req: IncomingMessage,
): Promise<{ fields: Record<string, string>; files: Record<string, { name: string; data: Buffer }> }> => {
  return new Promise((resolve, reject) => {
    const chunks: Buffer[] = [];
            ...
            if (dispositionLine) {
              ...
              if (nameMatch) {
                ...
                if (filenameMatch) {
                  ...
                  const fileData = Buffer.from(part.substring(contentStartIndex, contentEndIndex), 'binary');
                  files[fieldName] = { name: filename, data: fileData };
                } else {
                  const value = part.substring(contentStartIndex, contentEndIndex);
